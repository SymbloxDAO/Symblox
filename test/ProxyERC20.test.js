...
contract("ProxyERC20", async (accounts) => {
  ...
  it("Must pass through to underlying via fallback function and emit on proxy", async () => {
    ...
    assert.equal(
      topics[3],
      web3.eth.abi.encodeParameter("bytes32", toBytes32("SNX"))
    );
  });
  ...
  describe("ProxyERC20 should adhere to ERC20 standard", async () => {
    ...
    it("should be able to call ERC20 transferFrom", async () => {
      ...
      // Check Balances
      assert.bnEqual(
        account1BalanceBefore.sub(amountToTransfer),
        account1BalanceAfter
      );
      assert.bnEqual(
        account3BalanceBefore.add(amountToTransfer),
    ...
    it("should be able to call ERC20 transfer", async () => {
      ...
      assert.bnEqual(
        account2BalanceBefore.add(amountToTransfer),
  ...
  describe("when third party contracts interact with our proxy", async () => {
    ...
    it("should be able to query ERC20 balanceOf", async () => {
      ...
      const thirdPartybalance = await thirdPartyExchanger.checkBalance(
        account1
      );
    ...
    it("should be able to transferFrom ERC20", async () => {
      ...
      const approveTX = await proxyERC20.approve(
        thirdPartyExchanger.address,
        amountToTransfer,
        {
          from: account1,
        }
      ...
      // should be able to query ERC20 allowance
      const allowance = await proxyERC20.allowance(
        account1,
        thirdPartyExchanger.address
      );
      ...
      await thirdPartyExchanger.doTokenSpend(
        account1,
        account2,
        amountToTransfer
      );
