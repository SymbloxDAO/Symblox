...
describe("PriceOracle test", function () {
  ...
  it("check price for WBTC, WETH", async function () {
    ...
    for (let i = 0; i < tokens.length; i++) {
      let amounts = await this.dexRouter.getAmountsOut(bigNum(1, decimals[i]), [
        tokens[i],
        params.USDCAddress,
      ]);
      ...
      expect(smallNum(amounts[1], 6)).to.be.closeTo(smallNum(price, 18), 0.001);
    }
  });

  it("updateBaseToken", async function () {
    expect(await this.PriceOracle.baseToken()).to.be.equal(params.USDCAddress);
    ...
    expect(await this.PriceOracle.baseToken()).to.be.equal(params.WBTCAddress);
  });
});
